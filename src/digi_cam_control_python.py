...
class Camera:
    """
    Python interface for the open source digicam-control software.
    Initialize the program by specifying where digiCamControl is installed.
    If left empty, the default location (C:/Program Files (x86)/digiCamControl)
    will be assumed.
    If openProgram is set to true, digiCamControl will automatically be opened in the background.
    """
    ...
    def capture(self, location: str = "") -> int | str:
        """
        Takes a photo - filename and location can be specified in string location,
    ...
    def __set_cmd(self, cmd: str, value: str) -> int:
        ...
        :return: Value indicating the success of the operation
        """
        r = subprocess.check_output(f"cd {self.exe_dir} \
                                    && CameraControlRemoteCmd.exe /c set {cmd} {value}",
                                    shell=True).decode()
        if 'null' in r:  # Success
    ...
    def __list_cmd(self, cmd: str) -> int | list[str]:
        ...
        """
        r = subprocess.check_output(f"cd {self.exe_dir} \
                                    && CameraControlRemoteCmd.exe /c list {cmd}",
                                    shell=True).decode()
        if 'Unknown parameter' in r:  # Error
            return_value = r[109:]
            print(f"Error: {return_value}")  # Format output message
